                    ...
                    $.ajax({
                        ...
                        success: function(respuesta) {
                            if (respuesta == 1) {
                                swal(
                                    "Eliminado!",
                                    "Se dio de baja al participante.",
                                    "success"
                                ).then((val) => {
                                    if (val == true) {
                                        location.reload();
                                ...
                                swal(
                                    "Error!",
                                    "El participante no tiene registrado actividades.",
                                    "error"
                                );
                            }
                        },
    ...
    seleccionaTipoDoc();*/
        ...
        $("#pais").change(function(event) {
            ...
            if (event.target.value == "PERU") {
                $("#cboDepartamento, #dpto, #provincia, #distrito").removeAttr(
                    "disabled",
                    "disabled"
                );
                ...
                $("#cboDepartamento, #dpto, #provincia, #distrito").append(
                    "<option value='0'>SELECCIONE</option>"
                );
                $("#cboDepartamento, #dpto, #provincia, #distrito").attr(
                    "disabled",
                    "disabled"
                );
                $("#cboDepartamento, #dpto, #provincia, #distrito").removeAttr(
      ...
      console.log(formAction);*/
        ...
        $dni_doc.bind("change", function(e) {
            ...
            var pos = this.value;
            var url =
                baseURL("") + "getDNI/" + e.target.value + "/" + evento + "";
            console.log(url);
            ...
            $.get(url, function(resp, depa) {
                if (resp.datos.length > 0) {
                    ...
                    if (
                        (resp.datos[0].eventos_id == evento &&
                            resp.datos[0].tipo_id == 2) ||
                        (resp.datos[0].eventos_id == evento &&
                            resp.datos[0].tipo_id == 1) ||
                        (resp.datos[0].eventos_id == evento &&
                            resp.datos[0].estudiantes_tipo_id != 7)
                    ) {} else {
                        console.log("Valid dni");
                        let g =
                            resp.datos[0].grupo == null ?
                            0 :
                            resp.datos[0].grupo == "" ?
                            0 :
                            1;
        ...
        $nom_curso.bind("change", function(e) {
            ...
            var url =
                baseURL("") + "getCurso/" + e.target.value + "/" + evento + "";

            $.get(url, function(resp, depa) {
                if (resp.datos.length > 0) {
                    $("#nom_curso2, #nombre_curso").val(
                        resp.datos[0].nom_curso
                    );
        ...
        $(".openHTML_2").click(function(e) {
            ...
            $.get(url, function(resp, resul) {
                ...
                let img =
                    ...
                    "images/g/" +
                    resp.plantillahtml +
                    '.jpg" class="img-fluid" alt="gafete modelo 1">';
        ...
        $(".eCorreos").click(function() {
            ...
            $.get(url, function(resp, resul) {
                if (resp.length > 0) {
                    ...
                    swal(
                        "Correos Enviados",
                        "Cantidad de Correos Procesados: " + resp.length + "",
                        "success"
                    );
                } else {
                    swal(
                        "Aviso",
                        "...no existen estudiantes en esta programación."
                    );
                    console.log("Menor");
                }
            });
        ...
        $inputdni.bind("change", function() {
            ...
            $.get(
                baseURL("estudiantes") + "/vdni/" + pos + "/" + evento + "",
                function(resp, depa) {
                    $("#dni_doc").val("");
                    ...
                    if (resp.existe == 0) {} else if (resp.existe == 2) {
                        swal(
                            "Advertencia",
                            "El DNI ya esta registrado en el evento.",
                            "warning"
                        );
                        $("#inputdni").val("");
                    } else {
                        ...
                        $("#inputApe_pat, #ap_paterno").val(
                            resp.data.ap_paterno
                        );
                        $("#inputApe_mat, #ap_materno").val(
                            resp.data.ap_materno
                        );
        ...
        $("form#estudiantesImportSave").submit(function(e) {
          ...
          if($("#cmbOrganizar" + x).val() == 14 ){
            ...
            if (_flag == 0) {
                ...
                swal({
                    type: "info",
                    title: "Por favor asignar al menos una columna",
            ...
            if (_flagDni == 0) {
                ...
                swal({
                    type: "info",
                    title: "El DNI es un campo obligatorio",
                    showConfirmButton: false,
                    timer: 2000,
                });
                $("#btnSumImport").removeAttr("disabled");
            ...
            $("#cargador_excel2").attr(
                "style",
                "display: block; position: -webkit-sticky;position: sticky;left: 0;"
            );
            e.preventDefault();
                ...
                success: function(respuesta) {
                    ...
                    if (respuesta == "error_no_plantilla") {
                        swal({
                            type: "info",
                            icon: "error",
                            title: "No existe plantilla ni formulario para este evento.",
                            showConfirmButton: false,
                            timer: 1500,
                        });
                        $("#btnSumImport").removeAttr("disabled");
        ...
        $("form#f_cargar_datos_estudiantes")
            .submit(function(event) {
                tipo_evento = $("#tipo_evento").val();
                if (tipo_evento == 2) {} else {
                    tipo_evento = 1;
                }
            ...
            .validate({
                ...
                submitHandler: function(form) {
                    ...
                    var actionform = $("#f_cargar_datos_estudiantes").attr(
                        "action"
                    );
                    console.log("actionform");
                    console.log(actionform);
                    ...
                    $.ajax({
                        ...
                        success: function(datos) {
                            ...
                            var _combo =
                                "<select class='form-control border-primary text-uppercase' required onchange='cmbOrganiza(this)' id='cambiar' name='cambiar' style='width:200px'>";
                            _combo = _combo + "<option value=0></option>";
                            _combo =
                                _combo +
                                "<option value=1>DNI / PASAPORTE</option>";
                            _combo =
                                _combo + "<option value=2>NOMBRES</option>";
                            _combo =
                                _combo +
                            ...
                            _combo =
                                _combo +
                                "<option value=18>DEPARTAMENTO</option>";
                            _combo =
                                _combo + "<option value=7>PROFESIÓN</option>";
                            _combo = _combo + "<option value=6>CARGO</option>";
                            _combo =
                                _combo +
                                "<option value=16>ENTIDAD / EMPRESA / ORGANIZACIÓN</option>";
                            _combo =
                                _combo +
                                "<option value=11>CORREO ELECTRÓNICO PERSONAL</option>";
                            _combo =
                                _combo + "<option value=10>CELULAR</option>";
                            _combo =
                                _combo + "<option value=9>TELEFONO</option>";
                            _combo =
                                _combo +
                                "<option value=15>EMAIL_LABOR</option>";
                            if (tipo_evento == 2) {
                                _combo =
                                    _combo +
                                    "<option value=19>INSCRITO</option>";
                                _combo =
                                    _combo +
                                    "<option value=20>ASISTIDO</option>";
                            }
                            /*SE QUITO*/
                    ...
                    return false;
                                ...
                                for (y = 0; y < datos.length; y++) {
                                    ...
                                    for (z = 0; z < _numCol; z++) {
                                        ...
                                        _strTable =
                                            _strTable +
                                            '<td style="background:#E5E5E5;" class="' +
                                            _clase +
                                            '">' +
                                            datos[y][z] +
                                            "</td>";
