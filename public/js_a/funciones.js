...
//ocultar busqueda avanzada
  ...
  $('#email_dominio').on('keyup change', function(e){
    let em = $('#email_dominio').val();
    if(em=="OTRO"){
      $("#email_dominio, #email").css('display','none').attr('required',false).val("");
      $("#xemail").val("");
      $("#actionSubmit").attr('disabled',false);
      $("#salida").removeClass('d-block').css('display','none');
      
      $("#email2").css('display','inline-block').attr('required',true);
    }
  });
